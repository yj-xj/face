...
class CameraProcessingThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            def async_process(frame, target):
                """后台异步处理"""
                try:
                    if self.face_swap_app.inswapper and self.face_swap_app.face_analyser:
                        h, w = frame.shape[:2]
                        if w > 480:
                            scale = 480 / w
                            small = cv2.resize(frame, (0, 0), fx=scale, fy=scale)
                            result = self.face_swap_app.insightface_face_swap(small, target)
                            if result is not None:
                                result = cv2.resize(result, (w, h))
                        else:
                            result = self.face_swap_app.insightface_face_swap(frame, target)
            ...
            while self.running:
                ...
                if self.processing_enabled and target_face is not None:
                    ...
                    if self.process_thread is None or not self.process_thread.is_alive():
                        self.process_thread = Thread(target=async_process, args=(frame.copy(), target_face))
                        self.process_thread.daemon = True
                        self.process_thread.start()

                    # 立即显示最新结果或原始帧
                    with self.result_lock:
                        display_frame = self.latest_result if self.latest_result is not None else frame
...
class EnhancedFaceSwapUI(QMainWindow):
    def __init__(self):
        ...
        if self.db_manager:
            ...
            print("数据库不可用，从本地文件夹加载...")
            self.loadFaceImagesFromLocal()
            self.loadVideoFiles()
    ...
    def loadFromDatabase(self):
        ...
        try:
            print("[数据库] 开始加载视频...")
            video_loader = self.db_manager.load_videos()
            if video_loader:
                self.load_threads.append(video_loader)
                video_loader.finished.connect(lambda: self.onLoadThreadComplete(video_loader))
                video_loader.error.connect(lambda: self.onLoadThreadComplete(video_loader))
                print("[数据库] 视频加载线程已启动")

            print("[数据库] 开始加载图片...")
            image_loader = self.db_manager.load_images()
            if image_loader:
                self.load_threads.append(image_loader)
                image_loader.finished.connect(lambda: self.onLoadThreadComplete(image_loader))
                image_loader.error.connect(lambda: self.onLoadThreadComplete(image_loader))
                print("[数据库] 图片加载线程已启动")
    ...
    def loadFaceImages(self):
        ...
        if self.db_manager:
            # 优先从数据库加载
            loader = self.db_manager.load_images()
            if loader:
                self.load_threads.append(loader)
                loader.finished.connect(lambda: self.onLoadThreadComplete(loader))
                loader.error.connect(lambda: self.onLoadThreadComplete(loader))
        else:
            # 如果没有数据库管理器，从本地加载
            self.loadFaceImagesFromLocal()

    def loadFaceImagesFromLocal(self):
        """从本地文件系统加载人脸图片"""
        try:
    ...
    def loadVideos(self):
        """从数据库加载视频列表"""
        if self.db_manager:
            loader = self.db_manager.load_videos()
            if loader:
                self.load_threads.append(loader)
                loader.finished.connect(lambda: self.onLoadThreadComplete(loader))
                loader.error.connect(lambda: self.onLoadThreadComplete(loader))
    ...
    def onLoadThreadComplete(self, thread):
        """加载线程完成回调"""
        if thread in self.load_threads:
            self.load_threads.remove(thread)

    def onVideosLoaded(self, videos):
        """视频加载完成回调"""
        print(f"[回调] onVideosLoaded 被调用，收到 {len(videos)} 个视频")
        try:
            self.video_list.clear()

            for video in videos:
                try:
                    # 使用local_path而不是file，因为本地处理需要访问本地文件系统路径
                    ...
                    try:
                        ...
                        if cap.isOpened():
                            ...
                            if ret:
                                ...
                                print(f"[缩略图] {video.get('original_filename')}")
                            cap.release()
                    except Exception as e:
                        print(f"[警告] 无法生成缩略图: {e}")
                        # 使用默认视频图标
                        item.setIcon(QIcon.fromTheme("video-x-generic"))

                    self.video_list.addItem(item)
                    print(f"[加载] 视频: {video.get('original_filename')}")
                except Exception as e:
                    print(f"[错误] 加载视频项失败: {e}")

            print(f"[完成] 共加载 {self.video_list.count()} 个视频")
            self.statusBar().showMessage(f"已加载 {self.video_list.count()} 个视频")

        except Exception as e:
            print(f"[错误] 处理视频列表失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def onImagesLoaded(self, images):
        """图片加载完成回调"""
        print(f"[回调] onImagesLoaded 被调用，收到 {len(images)} 个图片")
        try:
            self.face_list.clear()
            if hasattr(self, 'video_face_list'):
                self.video_face_list.clear()
            ...
            for image in images:
                try:
                    ...
                    item_camera.setText(image.get('original_filename', image.get('filename', 'Unknown')))
                    item_camera.setSizeHint(QSize(150, 160))
                    self.face_list.addItem(item_camera)
                    ...
                    if hasattr(self, 'video_face_list'):
                        ...
                        # 使用相同的图标
                        if item_camera.icon():
                            item_video.setIcon(item_camera.icon())

                        item_video.setText(image.get('original_filename', image.get('filename', 'Unknown')))
                        item_video.setSizeHint(QSize(150, 160))
                        self.video_face_list.addItem(item_video)

                    print(f"已加载图片: {image.get('original_filename')} -> {local_path}")
                except Exception as e:
                    print(f"加载图片项失败: {e}")

            total_count = self.face_list.count()
            print(f"共加载 {total_count} 个图片")
            self.statusBar().showMessage(f"已加载 {total_count} 个图片")
    ...
    def selectFaceImage(self, item):
        ...
        if hasattr(self, 'camera_thread') and self.camera_thread is not None and self.camera_thread.isRunning():
            ...
            # 自动启用换脸
            self.camera_thread.set_processing_enabled(True)
            self.camera_processing_check.setChecked(True)
    ...
    def processingFinished(self, output_path):
        ...
        # 保存输出视频到数据库
        if self.db_manager:
            self.saveOutputVideoToDatabase(output_path)
    ...
    def saveOutputVideoToDatabase(self, output_path):
        """保存输出视频到数据库"""
        try:
            import requests
            import cv2
            ...
            if not os.path.exists(output_path):
                print(f"输出视频不存在: {output_path}")
                return

            # 根据文件名查找对应的输入视频和人脸图片ID
            video_basename = os.path.basename(self.selected_video_path)
            face_basename = os.path.basename(self.selected_face_path)

            # 从数据库获取输入视频和人脸图片的ID
            try:
                # 获取输入视频ID
                response = requests.get(
                    f"{self.db_manager.base_url}/../videos/",
                    params={'original_filename': video_basename},
                    timeout=30
                )
                input_video_id = None
                if response.status_code == 200:
                    ...
                    for item in results:
                        if item.get('original_filename') == video_basename:
                            input_video_id = item.get('id')
                            break

                # 获取人脸图片ID
                response = requests.get(
                    f"{self.db_manager.base_url}/../images/",
                    params={'original_filename': face_basename},
                    timeout=30
                )
                face_image_id = None
                if response.status_code == 200:
                    ...
                    for item in results:
                        if item.get('original_filename') == face_basename:
                            face_image_id = item.get('id')
                            break

                if face_image_id:
                    # 保存输出视频
                    result = self.db_manager.save_output_video(
                        output_path,
                        input_video_id,
                        face_image_id,
                        'inswapper'
                    ...
                    if result:
                        print(f"输出视频已保存到数据库: {os.path.basename(output_path)}")
                        self.statusBar().showMessage(f"视频已保存到数据库: {os.path.basename(output_path)}")
                    else:
                        print(f"保存输出视频到数据库失败")
                else:
                    print(f"未找到人脸图片记录: {face_basename}")
    ...
    def startCamera(self):
        ...
        try:
            ...
            # 创建摄像头线程（不需要预先选择人脸）
            target_face = getattr(self, 'selected_face_path', None)
        ...
        try:
            ...
            if file_path:
                if self.db_manager:
                    ...
                    self.upload_video_btn.setText("上传中...")

                    # Upload and connect signals
                    uploader = self.db_manager.upload_video(file_path)
                    # 保存线程引用，防止被销毁
                    self.upload_threads.append(uploader)
                    uploader.finished.connect(lambda result, thread=uploader: self.onVideoUploadSuccess(result, thread))
                    uploader.error.connect(lambda error_msg, thread=uploader: self.onVideoUploadError(error_msg, thread))
                else:
    ...
    def onVideoUploadSuccess(self, result, thread):
        ...
        try:
            ...
            msg = "Video uploaded successfully! File: " + filename
            QMessageBox.information(self, "成��", msg)
            self.upload_video_btn.setEnabled(True)
            self.upload_video_btn.setText("[上传] 添加视频")
            print("Video uploaded: " + str(result))
    ...
    def onVideoUploadError(self, error_msg, thread):
        """Video upload error callback"""
        # 从列表中移除线程
        if thread in self.upload_threads:
            self.upload_threads.remove(thread)
        ...
        if file_path:
            ...
            if self.db_manager:
                ...
                if uploader:
                    ...
                    self.upload_threads.append(uploader)
                    # 连接信号
                    uploader.finished.connect(lambda result, thread=uploader: self.onImageUploadSuccess(result, thread))
    ...
    def onImageUploadSuccess(self, result, thread):
        ...
        try:
            ...
            filename = result.get("original_filename", "Unknown")
            QMessageBox.information(self, "成功", f"图片上传成功！文件: {filename}")
            # 重新加载图片列表
            self.loadFaceImages()
        except Exception as e:
            print(f"Error in success callback: {e}")

    def onImageUploadError(self, error_msg, thread):
    ...
    def uploadFaceImage(self):
        """Upload face image to backend"""
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            "Select face image to upload",
            "",
            "Image Files (*.jpg *.jpeg *.png *.bmp);;All Files (*)"
        )
        if file_path:
            ...
            if self.db_manager:
                ...
                if uploader:
                    # 保存线程引用，防止被销毁
                    self.upload_threads.append(uploader)
                    uploader.finished.connect(lambda result, thread=uploader: self.onFaceImageUploadSuccess(result, thread))
                    uploader.error.connect(lambda error_msg, thread=uploader: self.onImageUploadError(error_msg, thread))
            else:
                QMessageBox.warning(self, "Error", "Database manager not initialized")
    ...
    def onFaceImageUploadSuccess(self, result, thread):
        ...
        try:
            ...
            filename = result.get("original_filename", "Unknown")
            QMessageBox.information(self, "成功", f"人脸图片上传成功！文件: {filename}")
            # 重新加载图片列表
            self.loadFaceImages()
        except Exception as e:
            print(f"Error in success callback: {e}")
...
def main():
    ...
    * {
    ...
    QComboBox {
        background-color: rgba(42, 46, 54, 0.8);
        border: 1px solid #3a3f4b;
        border-radius: 4px;
        padding: 5px 10px;
        color: #d0d0d0;
    ...
    QTabBar::tab:selected {
        background-color: rgba(58, 62, 75, 0.9);
        color: #ffffff;
        border-bottom: 1px solid rgba(58, 62, 75, 0.9);
    }

    QTabBar::tab:hover {
        background-color: rgba(58, 62, 75, 0.7);
    }
